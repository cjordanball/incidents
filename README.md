...
2. These keys will be discontinued in the near future, but please keep the repo private just to keep these keys safe.
3. In the back-end/data directory, place the JSON files for the incident reports as desired.
4. In the terminal, go to the root level of the back-end directory and run *yarn run start*, which should set up the server. You should see the log message "listening at http://localhost:3300" in the terminal.
5. Open a separate terminal tab, navigate to the root level of the front-end directory, and run *yarn run start*, then open a browswer tab (if it doesn't open automatically) to "localhost:3000". The Incident Report page should appear. 
